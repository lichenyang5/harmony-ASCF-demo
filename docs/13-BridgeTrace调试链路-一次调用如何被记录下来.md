...
这一节有一个**重要前提**要先说清：本项目里的 H5 跑在鸿蒙 WebView 里，`window.ascfBridge` 是 ArkTS 通过 `javaScriptProxy` 注入的对象。把 [ascf_bridge_demo.html](../entry/src/main/resources/rawfile/ascf_bridge_demo.html) 用桌面 Chrome 直接打开 —— **里面不会有 `ascfBridge` 这个全局** —— `ascfBridge.send(...)` 会立刻报 `Cannot read properties of undefined`。
